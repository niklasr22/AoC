...
scanners = list(map(lambda x : list(map(lambda y : list(map(int, y.split(","))), x.splitlines()[1:])), data.split("\n\n")))
fixed_beacons = set()
for i in scanners[0]:
        ...
        for scanner in unknownScanners:
            ...
            for rotated_scan in possible_scanner_rotations:
                for rs_beacon in rotated_scan:
                    ...
                    shifted_rotated_scan = {(x+dx, y+dy, z+dz) for x,y,z in rotated_scan}
