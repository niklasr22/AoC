...
def get_no_depending(block, removed_blocks):
    depending_blocks = graph[block]

    removed_blocks.add(block)

    truly_depending = []
    for dep_b in depending_blocks:
        if len([b for b in supported_by_graph[dep_b] if b not in removed_blocks]) == 0:
            truly_depending.append(dep_b)

    removed_blocks.update(truly_depending)
    for tdep_b in truly_depending:
        get_no_depending(tdep_b, removed_blocks)
    return removed_blocks


total_falling_blocks = 0
for block in settled:
    falling_blocks = len(get_no_depending(block, set())) - 1
    total_falling_blocks += falling_blocks

print(total_falling_blocks)
