...
valves = {v[6:8]: {"flow": int(v[23:v.find(";")]), "neighbours": v[v.find(" ", v.find("valve"))+1:].split(", ")} for v in valves}
    ...
    for next in new_closed_valves:
        if time + new_weights[(node, next)] < AVAILABLE_TIME:
            press = open_valve(
                ...
                pressure=pressure + new_weights[(node, next)] * current_flow, # pressure that is released while traveling
                node=next, 
                closed_valves=new_closed_valves
            )
            new_max_pressure = max(new_max_pressure, press)
        else:
            new_max_pressure = max(new_max_pressure, pressure + (AVAILABLE_TIME - time) * current_flow)
...
for l in range(1, len(reduced_valves)):
    for el_set in itertools.combinations(reduced_valves, l):
        ...
        for starting_valve in el_set:
            press_el = open_valve(dists[("AA", starting_valve)], 0, 0, starting_valve, el_set)
            max_press_el = max(max_press_el, press_el)
        for starting_valve in me_set:
            press_me = open_valve(dists[("AA", starting_valve)], 0, 0, starting_valve, me_set)
