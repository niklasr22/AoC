...
for (sx, sy), d in sensors.items():
    ...
    desc_lines.append((sx - d - 1, sy, sx, sy - d - 1, points_to_lin(sx - d - 1, sy, -1)))
    desc_lines.append((sx, sy + d + 1, sx + d + 1, sy, points_to_lin(sx, sy + d + 1, -1)))
...
for al in asc_lines:
    for dl in desc_lines:
        ...
        if dl[0] <= intersection_x <= dl[2] and 0 <= intersection_x <= MAX_COORD_VAL and 0 <= intersection_y <= MAX_COORD_VAL:    
            candidates.add(isec)
