...
def check_for_complete_overlap(pair: str) -> int:
    ...
    if (e1l in range(e2l, e2u + 1) and e1u in range(e2l, e2u + 1)) or (
        e2l in range(e1l, e1u + 1) and e2u in range(e1l, e1u + 1)
    ):
        return 1
    return 0
...
def check_for_overlap(pair: str) -> int:
    e1l, e1u, e2l, e2u = prepare_data(pair)
    if (
        e1l in range(e2l, e2u + 1)
        or e1u in range(e2l, e2u + 1)
        or e2l in range(e1l, e1u + 1)
        or e2u in range(e1l, e1u + 1)
    ):
        return 1
    return 0
